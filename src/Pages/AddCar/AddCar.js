...
import { bucket, db } from "../../util/config";
...
export default function AddCar(props) {
  ...
  const onSubmit = async (data) => {
    ...
    images.map((image) => {
      promises.push(bucket.ref(`c_cars/${vnumber}/${uuidv4()}`).put(image));
    ...
    Promise.all(promises)
      .then((results) => {
        ...
        Promise.all(promises).then(async (results) => {
          ...
          try {
            const result = await db.collection("c_cars").doc(vnumber).set({
              selectedDate,
              ...
              >
                ...
                <Grid item md={3} sm={6} xs={12}>
                  <TextField
                    ...
                    helperText={
                      errors.conumber ? errors.conumber.message : null
                    }
                  />
                </Grid>
                <Grid item md={3} sm={6} xs={12}>
